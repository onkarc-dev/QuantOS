...
function bucket(values:number[]){const rows=[{name:'Big Loss',trades:0},{name:'Small Loss',trades:0},{name:'Flat',trades:0},{name:'Good Win',trades:0},{name:'Big Win',trades:0}]; values.forEach(v=>{if(v<-1)rows[0].trades++; else if(v<0)rows[1].trades++; else if(v<1)rows[2].trades++; else if(v<2)rows[3].trades++; else rows[4].trades++;}); return rows}
function advice(data:any){if(!data)return 'Select a completed job to see analytics advice.'; const avg=Number(data.avg_R||0), wr=(Number(data.wins||0)/Math.max(1,Number(data.values?.length||0)))*100; if(avg>0.15&&wr>35)return 'This test shows positive expectancy. Validate on more symbols and time periods before changing rules.'; if(avg<0)return 'This result is not ready. Focus on reducing weak setups, deep drawdown, and false breakouts.'; return 'This result is borderline. Keep rules stable and increase sample size before judging.'}
...
export default function Analytics(){
  ...
  return <>
    <div className="hero"><h1>Advanced Analytics</h1><p className="muted">Visual report for equity, R distribution, win/loss mix, and strategy advice.</p></div>
    ...
    {data&&<>
      <div className="grid">
        <div className="card"><h3>Gross R</h3><div className="metric" style={{color:Number(data.gross_R)>=0?'#22c55e':'#ef4444'}}>{fmt(data.gross_R)}R</div></div>
        <div className="card"><h3>Avg R</h3><div className="metric" style={{color:Number(data.avg_R)>=0?'#22c55e':'#ef4444'}}>{fmt(data.avg_R,3)}R</div></div>
        <div className="card"><h3>Wins / Losses</h3><div className="metric"><span style={{color:'#22c55e'}}>{wins}</span>/<span style={{color:'#ef4444'}}>{losses}</span></div></div>
      ...
      <div className="grid">
        ...
        <div className="card"><h2>R Distribution</h2><p className="muted">Shows if losses are controlled and winners are meaningful.</p><ResponsiveContainer width="100%" height={240}><BarChart data={dist}><CartesianGrid strokeDasharray="3 3" stroke="#334155"/><XAxis dataKey="name" stroke="#cbd5e1"/><YAxis stroke="#cbd5e1"/><Tooltip contentStyle={tip}/><Bar dataKey="trades" fill="#a78bfa" radius={[8,8,0,0]}/></BarChart></ResponsiveContainer></div>
        <div className="card"><h2>Win/Loss Mix</h2><p className="muted">Green wins, red losses, purple flat trades.</p><ResponsiveContainer width="100%" height={240}><PieChart><Pie data={pie} dataKey="value" nameKey="name" outerRadius={82} label>{pie.map((x,i)=><Cell key={i} fill={x.color}/>)}</Pie><Tooltip contentStyle={tip}/></PieChart></ResponsiveContainer></div>
      </div>
